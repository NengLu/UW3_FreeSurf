...
# ### Crameri2012Case1_FreeSurface
# 
# This is the case1 (featuring a cosine perturbation of the surface) from the reference (Crameri et al., 2012), results see Fig.2.
# 
# <center>
# Numerical setups used in this work. (motified from table 1 in (Crameri et al., 2012))
# </center>
# 
# |Top boundary|Grid points | Tracers   |
# | ----------- | ----------- |----------- |
# |Stick air | 560 $\times$ 320|36 part./elem. |Tracers  
# |Free surface  | 256 $\times$ 64  | 36 part./elem.|
# 
# ### Reference
# 
...
ElementType_case = int(sys.argv[2])
...
vdegree  = int(ElementType_set[ElementType_case][0])
pdegree = int(ElementType_set[ElementType_case][1])
pcont = ElementType_set[ElementType_case][2]
...
max_time  = ndim(1e5*u.year)
dt_set    = ndim(5e2*u.year)
save_every = int(ndim(1e3*u.year)/dt_set)

xmin, xmax = ndim(0 * u.kilometer), ndim(2800 * u.kilometer)
ymin, ymax = ndim(0 * u.kilometer), ndim(700 * u.kilometer)
# boxl = xmax-xmin
# boxh = ymax-ymin
...
outputPath = f'op_Crameri2012Case1_xres{xres}_{ElementType_name[ElementType_case]}/'
if uw.mpi.rank == 0:
...
mesh = uw.meshing.StructuredQuadBox(elementRes=(int(xres), int(yres)), minCoords=(xmin, ymin), maxCoords=(xmax, ymax),qdegree=max(pdegree, vdegree))
init_mesh = uw.meshing.StructuredQuadBox(elementRes=(int(xres), int(yres)), minCoords=(xmin, ymin), maxCoords=(xmax, ymax),qdegree=max(pdegree, vdegree))
#mesh.view()
...
hmax =  1.0  #nd(700* u.kilometer)
hLith = ndim(100.* u.kilometer)

wavelength = ndim(2800.* u.kilometer)
amplitude  = ndim(7*u.kilometer)
k = 2. * np.pi / wavelength

def perturbation(x):
    return amplitude*np.cos(k*x)+hmax
...
with swarm.access(material):
    ...
    material.data[:, 0] = np.where(swarm.particle_coordinates.data[:, 1] < hmax-hLith, MantleIndex, LithIndex)
    
viscM = ndim(1e21 * u.pascal * u.second)
viscL = ndim(1e23 * u.pascal * u.second)    
    
densityM = ndim(3300 * u.kilogram / u.metre**3)   # for an
densityL = ndim(3300 * u.kilogram / u.metre**3)   # for litho    
    
density_fn = material.createMask([densityM, densityL])
visc_fn = material.createMask([viscM,viscL])
...
#while step < max_steps:
    ...
    if uw.mpi.rank == 0:
        particle_coordinates = mesh.data[topwall]
        w = particle_coordinates[:,1].max()
    ...
    swarm.advection(V_fn=v.fn, delta_t=dt,evalf=True)
    #swarm.advection(V_fn=v.fn, delta_t=dt)
