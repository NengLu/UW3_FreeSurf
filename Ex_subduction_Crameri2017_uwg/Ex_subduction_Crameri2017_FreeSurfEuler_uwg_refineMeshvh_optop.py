...
Model.outputDir= "output_fsurfEuler_yres{}_uwg_refinevh_opentop".format(yRes)
...
with Model.mesh.deform_mesh():
    ...
    normYs = -1.* Model.mesh.data[:,1]/1.0
    index = np.where(Model.mesh.data[:,1]>=0)
    normYs[index] = 1.* Model.mesh.data[index,1]/ Model.mesh.maxCoord[1] 
